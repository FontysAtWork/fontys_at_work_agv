...
path = '/sys/class/PWM/PWM2/ENABLE'
d = os.open(path, os.O_WRONLY)

def init():
    os.chdir(cmd_dir)
    Popen('printf 0 > ENABLE', shell=True)
    rospy.init_node('dir_out_lb', anonymous=True)
    rospy.Subscriber('/direction_wheel_lb', Int32, vec_to_pwm)
...
if __name__ == '__main__':
    try:
        ...
        rospy.signal_shutdown('dir_out_lb')
        pass